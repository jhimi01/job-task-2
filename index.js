...
function increaseStrokeDashArray(length) {
  // Select the SVG path element with class "transrgwht"
  let pathElement = document.querySelector(".transrgwht");
  ...
  if (pathElement) {
    // pathElement?.style?.fill="red";
    // Get the current value of stroke-dasharray
    // let currentDashArray = pathElement.getComputed("stroke-dasharray");

    let computedStyle = window.getComputedStyle(pathElement);

    let dashArray = computedStyle.getPropertyValue("stroke-dasharray");

    let dashArrayValues = dashArray.split(", ");

    dashArrayValues[0] = length * baseLength;

    let newDashArray = dashArrayValues.join(", ");
